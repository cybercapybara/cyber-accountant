...
Both layers name what was lost: which fixture, which value or label, which
word crossed which margin by how much.
...
def read_expectations(paths):
    """Parse the expectation files into (labels, unprinted, defects, margin).
    ...
    return labels, unprinted, defects, defect_labels, margin, margin_src
...
def check_content(pdf, fixture_path, labels, unprinted, defect_labels, template_path, source):
...
def main(argv):
    ...
    (labels, unprinted, defects, defect_labels,
    ...
    content, label_count, value_count = check_content(
        pdf, fixture_path, labels, unprinted, defect_labels, template_path, source)
    ...
    if content or geometry:
        ...
        return 1
    print("PASS %s — %d label(s), %d value(s), %d word box(es) inside the "
          "%.1fpt margin box%s"
          % (name, label_count, value_count, words, margin_pt,
