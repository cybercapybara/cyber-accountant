...
helm --kube-context "$CTX" upgrade --install "$RELEASE" "$CHART" \
    ...
    --set cyber-accountant.externalDatabase.password="$DB_PASS" \
    --set cyber-accountant.externalRedis.password="$REDIS_PASS" \
    --set cyber-accountant.auth.jwtSecret="$JWT_SECRET" \
    --set cyber-accountant-worker.externalDatabase.password="$DB_PASS" \
    --set cyber-accountant-worker.externalRedis.password="$REDIS_PASS" \
    --set cyber-accountant-worker.auth.jwtSecret="$JWT_SECRET" \
    --wait --timeout 10m
...
POD="$(kubectl --context "$CTX" -n "$NS" get pod -l app.kubernetes.io/name=cyber-accountant \
    -o jsonpath='{.items[0].metadata.name}')"
# config path is positional and must precede the mode flag.
kubectl --context "$CTX" -n "$NS" exec "$POD" -- \
    /app/cyber_accountant config/config.json --create-admin "$ADMIN_EMAIL" "$ADMIN_PASS" || true
kubectl --context "$CTX" -n "$NS" exec "$POD" -- \
    /app/cyber_accountant config/config.json --seed-fake 8 || true
