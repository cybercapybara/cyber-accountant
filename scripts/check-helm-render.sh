...
#    cyber-accountant.mail.enabled, so a missing enabled silently drops it (mail breaks).
[ "$(q 'select(.kind=="Deployment" and .metadata.name=="api") | .spec.template.spec.containers[0].env[] | select(.name=="MAIL_VIA_JOBS") | .value')" = "true" ] ||
    fail "MAIL_VIA_JOBS not rendered =true on api (set cyber-accountant.mail.enabled=true)"
...
#    it. Render the cyber-accountant chart with the TRACKED prod example overlay and
#    assert the security-critical env the cluster actually runs with.
echo "==> helm template (cyber-accountant prod example) + security assertions"
API_CHART="$ROOT/helm/cyber-accountant"
