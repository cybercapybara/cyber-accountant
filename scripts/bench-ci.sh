...
#   docker build --target runtime -f docker/Dockerfile -t cyber-accountant:bench .
#   ./scripts/bench-ci.sh
set -euo pipefail

OUT_DIR="${OUT_DIR:-bench-out}"
BENCH_IMAGE="${BENCH_IMAGE:-cyber-accountant:bench}"
...
sleep 5
rss_mb=$(docker stats --no-stream --format '{{.MemUsage}}' cyber_accountant_app \
    | awk -F'[ /]+' '{v=$1
