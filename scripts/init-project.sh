...
if [[ -f project.env ]]; then
    ...
    # Default ships as PROJECT_NAME=cyber-accountant; treat that as "untouched template".
    if [[ -n "$EXISTING_NAME" && "$EXISTING_NAME" != "cyber-accountant" && "$EXISTING_NAME" != "cyber-accountant" ]]; then
...
declare -a PATTERNS=(
    ...
    "s|ghcr.io/cybercapybara/cyber-accountant|${REGISTRY}/${PROJECT_NAME}|g"
    # 2. CI image name
    "s|ghcr.io/cybercapybara/cyber-accountant|${REGISTRY}/${PROJECT_NAME}|g"
    ...
    "s|cyber-accountant|${PROJECT_NAME}|g"
    # 4. CMake project name, binary names, Dockerfile
    "s|cyber_accountant|${PROJECT_SNAKE}|g"
    # 5. Bench service name
    "s|cyber_accountant_bench|${PROJECT_SNAKE}_bench|g"
    # 6. OTel service name
    "s|cyber_accountant_service|${PROJECT_SNAKE}_service|g"
    # 7. Worker OTel service name
    "s|cyber_accountant_worker_service|${PROJECT_SNAKE}_worker_service|g"
    # 8. Helm maintainer team
    "s|cyber-accountant-team|${PROJECT_NAME}-team|g"
    # 9. Helm worker chart name
    "s|cyber-accountant-worker|${PROJECT_NAME}-worker|g"
    # 10. Helm chart, K8s, ingress
    "s|cyber-accountant|${PROJECT_NAME}|g"
    # 11. Worker logger name
    "s|cyber_accountant_worker|${PROJECT_SNAKE}_worker|g"
    # 12. Logger, containers, general snake_case
    "s|cyber_accountant|${PROJECT_SNAKE}|g"
    # 13. Kafka client ID
    "s|cyber_accountant_producer|${PROJECT_SNAKE}_producer|g"
...
if [[ $DRY_RUN -eq 1 ]]; then
    ...
    for f in "${FILES[@]}"; do
        if grep -qE 'cyber-accountant|cyber_accountant|cyber-accountant|cyber_accountant|cyber-accountant-worker|cyber_accountant_worker|cyber_accountant_producer|cyber_accountant_bench|cyber_accountant_service|cyber_accountant_worker_service|cyber-accountant-team|resert/|tarassov\.me|46\.225\.37\.165|YOUR_KUBE_CONTEXT|change-me-demo-pass' "$f" 2>/dev/null; then
            echo "    $f"
        fi
    done
    echo ""
    if [[ -d "helm/cyber-accountant" && "cyber-accountant" != "${PROJECT_NAME}" ]]; then
        echo "==> Would rename helm/cyber-accountant -> helm/${PROJECT_NAME}"
    fi
    if [[ -d "helm/cyber-accountant-worker" && "cyber-accountant-worker" != "${PROJECT_NAME}-worker" ]]; then
        echo "==> Would rename helm/cyber-accountant-worker -> helm/${PROJECT_NAME}-worker"
    fi
...
if [[ -d "helm/cyber-accountant" && "cyber-accountant" != "${PROJECT_NAME}" ]]; then
    mv "helm/cyber-accountant" "helm/${PROJECT_NAME}"
    echo "==> Renamed helm/cyber-accountant -> helm/${PROJECT_NAME}"
fi

if [[ -d "helm/cyber-accountant-worker" && "cyber-accountant-worker" != "${PROJECT_NAME}-worker" ]]; then
    mv "helm/cyber-accountant-worker" "helm/${PROJECT_NAME}-worker"
    echo "==> Renamed helm/cyber-accountant-worker -> helm/${PROJECT_NAME}-worker"
fi
...
# UNAMBIGUOUS author/template tokens are flagged: bare "cyber-accountant"/"cyber_accountant" are
# excluded so a fork named e.g. "cyber-accountant-gateway" doesn't trip it. Vendor /
...
LEFTOVER_RE='cyber-accountant|cyber_accountant|cyber_accountant_bench|cyber_accountant_service|cyber_accountant_worker_service|cyber_accountant_producer|cyber-accountant-team|resert/|ghcr\.io/resert|tarassov\.me|46\.225\.37\.165|YOUR_KUBE_CONTEXT|change-me-demo-pass|michael@tarassov\.me'
