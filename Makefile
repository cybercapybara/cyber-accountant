...
UPSTREAM_GHCR ?= ghcr.io/cybercapybara/cyber-accountant
...
test:              ## Run all tests in Docker (rebuild image, ~2 min cold)
	...
	$(COMPOSE) $(ENV) --profile test run --rm -e TEST_BINARY=cyber_accountant_tests_unit test-runner
	$(COMPOSE) $(ENV) --profile test run --rm -e TEST_BINARY=cyber_accountant_tests_integration test-runner
	@$(MAKE) --no-print-directory test-e2e

test-e2e:          ## Run the HTTP end-to-end binary (real Drogon server + client)
	$(COMPOSE) $(ENV) --profile test run --rm -e TEST_BINARY=cyber_accountant_e2e test-runner
...
test-quick:        ## Re-run tests against existing image (no rebuild, ~5 s)
	...
	@if docker image inspect $${TEST_IMAGE:-cyber-accountant:test-latest} >/dev/null 2>&1; then \
		$(COMPOSE) $(ENV) --profile test run --rm -e TEST_BINARY=cyber_accountant_tests_unit test-runner && \
		$(COMPOSE) $(ENV) --profile test run --rm -e TEST_BINARY=cyber_accountant_tests_integration test-runner ; \
	else \
...
warm-cache:        ## Pull a CI-built builder image to skip the cold vcpkg build (~30 min -> ~3)
	...
	@for ref in $(GHCR_REPO)/builder:cache $(UPSTREAM_GHCR)/builder:cache ; do \
		...
		if docker pull "$$ref" 2>/dev/null ; then \
			docker tag "$$ref" cyber-accountant:builder-latest ; \
...
test-local:        ## Run native gtest binary; pass NAME=<filter> to scope: make test-local NAME=Pagination*
	...
	@./build/$(or $(PRESET),dev)/cyber_accountant_tests_unit \
		--gtest_color=yes \
		$(if $(NAME),--gtest_filter=$(NAME))
	@./build/$(or $(PRESET),dev)/cyber_accountant_tests_integration \
		--gtest_color=yes \
		$(if $(NAME),--gtest_filter=$(NAME))
...
coverage:          ## Build with coverage, run tests, emit HTML + fail under COVERAGE_MIN% line coverage
	...
	@./build/coverage/cyber_accountant_tests_unit --gtest_color=yes || true
	@./build/coverage/cyber_accountant_tests_integration --gtest_color=yes || true
	@./build/coverage/cyber_accountant_e2e --gtest_color=yes || true
	@mkdir -p coverage
...
routes:            ## Print the registered endpoint table (no DB required)
	@$(COMPOSE) exec app ./cyber_accountant --print-routes 2>/dev/null || \
	    ./build/dev/cyber_accountant --print-routes 2>/dev/null || \
...
migrate-local:     ## Apply pending migrations natively (requires build-local + reachable Postgres)
	@if [ ! -x ./build/$(or $(PRESET),dev)/cyber_accountant ]; then \
		echo "==> No native build at build/$(or $(PRESET),dev)/ — running build-local first"; \
		$(MAKE) --no-print-directory build-local PRESET=$(or $(PRESET),dev); \
	fi
	./build/$(or $(PRESET),dev)/cyber_accountant --run-migrations $(or $(CONFIG),config/config.json)
