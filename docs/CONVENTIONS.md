...
15. **Never give a test fixture a real `observability.metrics_address`.** `TestHelpers::minimal_config()` leaves it empty on purpose: prometheus-cpp's `Exposer` is a CivetWeb HTTP server whose listener thread parks in `poll()` for a 2000 ms quantum, and `~Exposer` (`Metrics::shutdown`) blocks until that quantum expires. A per-test exposer therefore costs a FLAT ~2 s in TearDown no matter what the test does — that is exactly how 442 integration/api tests grew to 12 minutes of CI. An empty address builds the Prometheus registry (so every `create_counter`/`create_gauge` still works) and starts no server. The bind path is covered on a real port by `tests/unit/test_observability.cpp`; nothing else scrapes `/metrics` (there is no Drogon route for it).
