...
2. Workers alive and consuming? Check `up{job="cyber_accountant_worker"}` and the worker
...
```sh
# 1. Fetch the dump
aws s3 cp s3://<bucket>/cyber-accountant/appdb-<ts>.sql.gz .
...
- k8s: `helm rollback cyber-accountant <REVISION>` (`helm history cyber-accountant`).
