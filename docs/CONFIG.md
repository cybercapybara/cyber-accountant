...
| Env | JSON key | Type | Default | Notes |
|---|---|---|---|---|
| `LOG_NAME` | `logging.name` | string | `cyber_accountant` | |
...
| `SERVICE_NAME` | `observability.service_name` | string | `cyber_accountant_service` | Also emitted as `service` field in JSON logs |
...
| `KAFKA_PRODUCER_ID` | `messaging.kafka.producer.client_id` | string | `cyber_accountant_producer` | |
...
## Worker (second binary, `cyber_accountant_worker`)
...
CONFIG_FILE=config/local.json ./cyber_accountant
```
