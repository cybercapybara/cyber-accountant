...
-- только метаданные. У аннулирования это осознанный дизайн (документ
-- остаётся скачиваемым), у удаления — настоящая утечка: после него ключ не
-- упомянут больше нигде и найти его невозможно.
--
-- Утечка ОТСЛЕЖИВАЕТСЯ, а не оставлена фольклором в комментарии:
-- https://github.com/cybercapybara/cyber-accountant/issues/11
-- (там же — почему удалять объект прямо в этой транзакции нельзя: S3 не
-- участвует в транзакции Postgres, и откат оставил бы строку, указывающую
-- на уже удалённые байты, что хуже осиротевшего объекта).
--
