...
[![CI](https://github.com/your-registry/your-project/cyber-accountant/actions/workflows/ci.yml/badge.svg)](https://github.com/your-registry/your-project/cyber-accountant/actions/workflows/ci.yml)
[![benchmarks](https://img.shields.io/badge/benchmarks-trend-blue)](https://cybercapybara.github.io/cyber-accountant/dev/bench/)
[![release](https://img.shields.io/github/v/release/your-registry/your-project/cyber-accountant)](https://github.com/your-registry/your-project/cyber-accountant/releases)
...
```bash
git clone https://github.com/your-registry/your-project/cyber-accountant.git my-service
cd my-service
...
docker compose -f docker/docker-compose.yml exec app \
    ./cyber_accountant --create-admin admin@local password
...
6. **Verify migrations didn't drift** — `docker compose exec app ./cyber_accountant --verify-migrations` exits
...
- `helm/cyber-accountant` — the HTTP service
- `helm/cyber-accountant-worker` — the background-job worker
...
```bash
helm template api helm/cyber-accountant --set image.repository=my-registry/cyber-accountant
helm template worker helm/cyber-accountant-worker --set image.repository=my-registry/cyber-accountant
```
...
```bash
cp helm/cyber-accountant/values-prod.example.yaml helm/cyber-accountant/values-prod.yaml   # gitignored
helm upgrade --install api ./helm/cyber-accountant -n prod -f helm/cyber-accountant/values-prod.yaml \
  --set externalDatabase.password="$DB_PASSWORD" \
  --set externalRedis.password="$REDIS_PASSWORD" \
  --set auth.jwtSecret="$JWT_SECRET"
# repeat for cyber-accountant-worker (its datastore/auth MUST match) and cpp-frontend
```
...
helm/            Helm charts (cyber-accountant, cyber-accountant-worker, cpp-frontend + cpp-env umbrella), values documented
