...
inline const std::vector<EndpointInfo>& get_endpoints() {
    static const std::vector<EndpointInfo> endpoints = {
        ...
        {"GET", "/api/v1/hr-orders", "List HR orders (optional ?employee_id filter)"},
        {"POST", "/api/v1/hr-orders", "Create an HR order (accountant/owner)"},
        {"POST", "/api/v1/hr-orders/{id}/generate-document", "Generate the hr_order document for an HR order (accountant/owner)"},
        ...
        {"POST", "/api/v1/labor-contracts/{id}/generate-document", "Generate the labor_contract document for a labor contract (accountant/owner)"},
        {"GET", "/api/v1/vacations", "List vacations (optional ?employee_id filter)"},
